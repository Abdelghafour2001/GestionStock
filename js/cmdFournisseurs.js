$(document).ready(function(){
	...
	$("#listeFournisseurs").on("change",function(e){
		if(verifierFournisseur()){
			creerCommande($("#listeFournisseurs option:selected").val());
		}
	})
});
// /////////////////////////////
function verifierFournisseur(){
	var idFournisseur = $("#listeFournisseurs option:selected").val();
	if(idFournisseur){
		if(idFournisseur === "-1"){
...
function creerCommande(idFournisseur){
	...
	$.getJSON("creerCommande", {
		...
		if(data){
			console.log("fournisseur a ete maj");
